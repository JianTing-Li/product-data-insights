...
describe('out-of-stock signal', () => {
  it('fires when available inventory is zero for a product with sales history', () => {
    ...
    const result = generateProductSignals(p, { dataQuality: emptyDataQuality() })
  ...
  it('does not fire when inventory is positive', () => {
    ...
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).not.toContain('out-of-stock')
  })
...
describe('restock-attention signal', () => {
  it('fires when days of inventory is low but positive', () => {
    ...
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).toContain('restock-attention')
  })
  ...
  it('does not fire when days of inventory is zero (that is out-of-stock territory)', () => {
    ...
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).not.toContain('restock-attention')
  })
...
describe('sales-decline signal', () => {
  it('fires on a significant revenue drop', () => {
    ...
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).toContain('sales-decline')
  })

  it('does not fire on a mild decline below the threshold', () => {
    const p = product({ salesChangePct: -0.05 })
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).not.toContain('sales-decline')
  })
...
describe('fast-growing signal', () => {
  it('fires on strong revenue growth', () => {
    ...
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).toContain('fast-growing')
  })

  it('does not fire on mild growth below the threshold', () => {
    const p = product({ salesChangePct: 0.05 })
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).not.toContain('fast-growing')
  })
})
...
describe('slow-moving-inventory signal', () => {
  it('fires when days of inventory is very high', () => {
    ...
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).toContain('slow-moving-inventory')
  })

  it('does not fire for a healthy days-of-inventory value', () => {
    const p = product({ hasInventoryData: true, daysOfInventory: 20 })
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).not.toContain('slow-moving-inventory')
  })
...
describe('margin-concern signal', () => {
  it('fires on thin gross margin', () => {
    ...
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).toContain('margin-concern')
  })

  it('does not fire on healthy margin', () => {
    const p = product({ grossMargin: 0.45 })
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).not.toContain('margin-concern')
  })
...
describe('reputation-concern signal', () => {
  it('fires on a low rating with meaningful volume', () => {
    ...
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).toContain('reputation-concern')
  })

  it('does not fire on a low rating with too little volume to be meaningful', () => {
    const p = product({ rating: 2.8, ratingCount: 3 })
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).not.toContain('reputation-concern')
  })
...
describe('promising-reputation signal', () => {
  it('fires on a high rating with meaningful volume', () => {
    ...
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).toContain('promising-reputation')
  })

  it('does not fire on a high rating with too little volume', () => {
    const p = product({ rating: 4.8, ratingCount: 10 })
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).not.toContain('promising-reputation')
  })
...
describe('price-integrity-risk signal', () => {
  it('fires when current price exceeds original price', () => {
    ...
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).toContain('price-integrity-risk')
  })

  it('does not fire for a normal discount (current below original)', () => {
    const p = product({ currentPrice: 30, originalPrice: 39 })
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).not.toContain('price-integrity-risk')
  })
...
describe('data-quality-hold signal', () => {
  it('fires when the product ID is unmatched in another dataset', () => {
    ...
    expect(idsOf(generateProductSignals(p, { dataQuality: dq }))).toContain('data-quality-hold')
  })
  ...
  it('does not fire for a clean product', () => {
    const p = product({ sku: 'X' })
    expect(idsOf(generateProductSignals(p, { dataQuality: emptyDataQuality() }))).not.toContain('data-quality-hold')
  })
})
